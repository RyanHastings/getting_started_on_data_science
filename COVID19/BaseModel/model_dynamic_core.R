...
for (t in 2:maxt) {
  ...
  for (i in 1:ncounties-1) {
    ...
    for (a in 1:nage) {
      ...
      dGdt[i,t,a]<-Rdeath[a]*term3-term6
      dQdt[i,t,a]<-(Rcrit-Rdeath[a])*term3-term7
      dHdt[i,t,a]<-(Rhosp[a]-Rcrit)*term3-term4
      dDdt[i,t,a]<-term6
      dRdt[i,t,a]<-term4+term7+(1-Rhosp[a])*term3
  ...
  if (statewide_method==1) {
    for (a in 1:nage) {
      ...
      Dday[ncounties,t,a]<-sum(Dday[1:ncounties-1,t,a])
    }
