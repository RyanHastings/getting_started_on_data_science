...
intervention_R_rdxn_vec<-seq(0.0,0.7,0.05) # this needs to match the range in StateSpace
Rhosp_vec<-seq(0.025,0.09,0.005)#0.06 # hospitalization rate..needs to match the range in StateSpace
Rcrit_vec<-seq(0.01,0.04,0.001)#0.024 # critical rate..needs to match the range in StateSpace

StateSpace_filename<-"StateSpace.Rdata" # StateSpace filename
...
DeathByDayVar<-4 # variable in StateSpace that is deaths per day
HospTotVar<-1 # variable in StateSpace that is hospital census
CritTotVar<-2 # variable in StateSpace that is ICU census
...
D.frame<-data.frame(dim1,dim2,dim3,day,R0=R0,InterventionReduction=intervention_R_rdxn,
